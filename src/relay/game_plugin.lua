...
local function run(args)
    ...
    while true do
        ...
        if r.channel == inbox then
            ...
            elseif topic == "resume" then
                -- Phase 5: SPA reconnected after wippy restart with a stored
                -- gameId. Ask game_manager whether the game is in-flight; if
                -- so it respawns the orchestrator with rehydrate=true.
                local game_id = tostring(data.game_id or "")
                if game_id == "" then
                    forward(conn_pid, "game_resume_failed", { code = "MISSING_GAME_ID" })
                else
                    local gm_pid = process.registry.lookup("app.game:game_manager")
                    if not gm_pid then
                        forward(conn_pid, "game_resume_failed", { code = "NO_GAME_MANAGER" })
                    else
                        -- Subscribe BEFORE sending game.resume so the
                        -- orchestrator's post-rehydrate game_state_changed
                        -- re-emit is not lost (mirrors start handler).
                        ensure_subscribed()
                        process.send(gm_pid, "game.resume", {
                            game_id = game_id,
                            driver_pid = process.pid(),
                        })
                        local wait_deadline = time.after("5s")
                        ...
                        while not got_reply do
                            ...
                            if not wait_r.ok or wait_r.channel == wait_deadline then
                                forward(conn_pid, "game_resume_failed", { code = "RESUME_TIMEOUT" })
                                unsubscribe()
                                break
                            end
                            local reply_msg = wait_r.value
                            local reply_topic = reply_msg and reply_msg:topic() or ""
                            if reply_topic == "game.resumed" then
                                local resumed = (reply_msg:payload() and reply_msg:payload():data()) or {}
                                local orch_pid = process.registry.lookup("game:" .. tostring(resumed.game_id))
                                if not orch_pid then
                                    forward(conn_pid, "game_resume_failed", { code = "ORCH_NOT_FOUND" })
                                    unsubscribe()
                                    got_reply = true
                                    break
                                end
                                active = {
                                    ...
                                    orch_pid = orch_pid,
                                    player_slot = resumed.player_slot or 1,
                                }
                                if resumed.player_role == "mafia" then
                                    ensure_mafia_subscribed()
                                end
                                logger:info("[game_plugin] resumed game",
                                    { game_id = resumed.game_id,
                                      player_role = resumed.player_role })
                                got_reply = true
                            elseif reply_topic == "game.resume_failed" then
                                local ferr = (reply_msg:payload() and reply_msg:payload():data()) or {}
                                forward(conn_pid, "game_resume_failed", {
                                    code = tostring(ferr.code or "RESUME_FAILED"),
                                })
                                unsubscribe()
                                got_reply = true
                            end
                            -- Other topics during the wait window are dropped.
                        end
                    end
                end
